...
import { Loader2, ShieldAlert, Bug, AlertTriangle, Activity, TrendingUp } from 'lucide-react';
...
export default function Reports() {
  ...
  const navSections = [
    ...
    {
      title: "INSIGHTS",
      items: ["Code Health Trend", "Issues Prevented", "Issue Distribution"]
    },
  ];
    ...
    <div className="flex bg-[#0E1116] h-[calc(100vh-130px)]">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="max-w-[1200px] w-full mx-auto">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-[14px] text-[#8B949E]">
                ...
                {activeTab === 'Issues Prevented' && 'Issues caught and prevented from reaching production.'}
                {activeTab === 'Issue Distribution' && 'How issues are distributed across categories.'}
              </p>
            </div>
          </div>
             ...
             <div className="flex gap-16">
                 <div>
                    ...
                    <span className={`flex items-center gap-1.5 text-[13px] font-bold tracking-wider ${
                      (activeTab === 'Security Overview' ? cats.security : activeIssues) > 0 ? 'text-[#f85149]' : 'text-[#3fb950]'
                    }`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${
                          (activeTab === 'Security Overview' ? cats.security : activeIssues) > 0 ? 'bg-[#f85149]' : 'bg-[#3fb950]'
                        }`}></div>
          ...
          {activeTab === 'Code Health Trend' && (
            <div className="border border-[#30363D] rounded-lg overflow-hidden flex divide-x divide-[#30363D]">
               ...
               <div className="flex-1">
                   ...
                   <div className="px-6 py-8 text-center text-xl font-bold text-[#3fb950]">{issuesPrevented}</div>
               </div>
               <div className="flex-1">
                   <div className="px-6 py-4 bg-[#161B22] border-b border-[#30363D]">
          ...
          {activeTab === 'Issues Prevented' && (
            <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-6">
              <div className="flex items-center gap-3 mb-6">
                <TrendingUp size={20} className="text-[#3fb950]" />
                <div>
                  <h3 className="text-[15px] font-semibold text-[#E6EDF3]">{issuesPrevented} issues prevented</h3>
                  <p className="text-[13px] text-[#8B949E]">Through {stats?.succeeded || 0} successful reviews</p>
                </div>
              </div>
              {issuesPrevented === 0 && (
                <p className="text-[13px] text-[#8B949E]">Issues prevented will increase as more PR reviews are completed successfully.</p>
              )}
            </div>
          )}

          {activeTab === 'Issue Distribution' && (
            <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-6">
              ...
              {categoryDistribution.length > 0 ? (
                <div className="space-y-4">
                  {categoryDistribution.map((cat, i) => (
                    <div key={i} className="flex items-center gap-4">
                      <span className="text-[13px] text-[#E6EDF3] w-32 capitalize">{cat.name}</span>
                      <div className="flex-1 h-6 bg-[#21262D] rounded-full overflow-hidden">
                        <div className="h-full bg-[#58a6ff] rounded-full flex items-center justify-end pr-2 text-[10px] font-bold text-white transition-all" 
                          style={{ width: `${Math.max((cat.count / (categoryDistribution[0]?.count || 1)) * 100, 8)}%` }}>
                          {cat.count}
                        </div>
                      </div>
                    </div>
                ...
                <div className="text-center py-12 text-[#8B949E]">
                  <Activity size={40} className="mx-auto mb-3 text-[#30363D]" />
                  <p className="text-sm">No issue distribution data yet.</p>
                </div>
              )}
            </div>
          )}
